...
import { edit } from "@/actions/action.product";
import { getProductBySlug } from "@/services/product/service.product";
...
export default async function ProductEditPage({ params, searchParams }: {
  ...
  const product = { ...raw, price: raw.price.toString() };
  ...
  return (
    <div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <form action={edit}>
          <div className="p-6">
            <ProductFormClient product={product} showSuccess={success === "true"} />
          </div>
